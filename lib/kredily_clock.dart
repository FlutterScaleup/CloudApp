...
class KredilyClock {
  ...
  static String taskBodyString =
      "Please add your tasklist in slack group- Team cloud tech";
  ...
  getKredily() async {
    String url =
        "https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com";
  ...
  loginKredily(String token) async {
    ...
    var response = await client.post(
        Uri.parse(
            'https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/login/'),
        headers: header,
        body: data);
    print(response.statusCode);
    print(response.body);
  ...
  getEmpDetails(csrfToken, sessionId) async {
    var header = {'Cookie': 'csrftoken=$csrfToken; sessionid=$sessionId'};

    var response = await http.get(
        Uri.parse(
            "https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/attendanceLog/clockingWidgetApi/"),
        headers: header);
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        if (myController.isLoginPage == true) {
          ...
          FirebaseMessaging.instance.subscribeToTopic(myController
        ...
        if (myController.shouldGoForward.value == true) {
          ...
          } else if (json['attendance_log'][0]['emp_time_out'] == null) {
          ...
          return [
            json['attendance_log'][0]['emp_time_out'] == null
                ? 'loggedIn'
                : 'loggedOut'
          ];
        }
  ...
  setClockin(csrfToken, sessionId) async {
    ...
    var response = await http.post(
        Uri.parse(
            "https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/attendanceLog/clockIn/"),
        headers: header);
    print(response.statusCode);
  ...
  setClockOut(csrfToken, sessionId) async {
    ...
    var response = await http.get(
        Uri.parse(
            "https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/attendanceLog/clockOut/"),
        headers: header);
    print(response.statusCode);
    if (response.statusCode == 201) {
      //created
  ...
  getLeaveStatus(csrfToken, sessionId) async {
    ...
    try {
      var response = await http.get(
          Uri.parse(
              'https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/leave-request/leave_accrual_user/'),
          headers: header);
      print(response.statusCode);
      var json = jsonDecode(response.body);
  ...
  getLeaveLogs(csrfToken, sessionId, years) async {
    ...
    var response = await http.get(
        Uri.parse(
            'https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/leave-request/user_leave_log_datatable?year=$years'),
        headers: header);
    // print(response.body);
    var json = jsonDecode(response.body);
  ...
  cancelLeave(csrfToken, sessionId, leaveRequestUu) async {
    ...
    var request = http.Request(
        "GET",
        Uri.parse(
            'https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/leave-request/cancelLeave/'));
    request.body = data;
  ...
  applyLeave(csrfToken, sessionId, leaveType, startDate, startDaySession,
      endDate, endDaySession, reasonForLeave) async {
    var header = {
      "X-CSRFToken": '$csrfToken',
      'Cookie': 'csrftoken=$csrfToken; sessionid=$sessionId',
      'Content-Type': 'application/json'
    ...
    try {
      var response = await http.post(
          Uri.parse(
              'https://zpmamyhz0m.execute-api.ap-south-1.amazonaws.com/scaleupallyio.kredily.com/leave-request/add-new/'),
          headers: header,
          body: data);
      print(response.body);
      ...
      if (json['status'] == "success") {
        Fluttertoast.showToast(
            msg: 'Leave Applied : status ${json['leave_obj']['status']}');
        return "success";
      }
    } catch (e) {
      print(e);
  ...
  sendNotification2() {
    ...
    Map<String, dynamic> message = {
      'to':
          '/topics/topic_name', // Replace with the topic or device token you want to send the message to
      'notification': {
    ...
    var message2 = jsonEncode({
      'to':
          '/topics/scaleup', // Replace with the topic or device token you want to send the message to
      'notification': {
        'title': 'Title of the notification',
        'body': 'Body of the notification'
      },
  ...
  sendNotification(title, body, topic) async {
    ...
    var headers = {
      "Authorization":
          "key=AAAA2l6H44s:APA91bHuB6WroEyzaWOMOfP13hn4CCe5UefvDJ4tFjj0xh8SdBrQeIl0csEqHxJ4mFCoGhoyT7rBUb9i4FOKNXYGy2t9CPUDhfMbiesQxUaUsBYgt5sTD_tbek3HgPhqk_ZX34kmXDNS",
      "Content-Type": "application/json"
    };
    var message = jsonEncode({
      'to':
          '/topics/$topic', // Replace with the topic or device token you want to send the message to
